...
import { MailOpen, Filter } from 'lucide-react';
          ...
          {filteredMessages.map((msg) => (
            ...
            >
              ...
              <div className="flex flex-wrap gap-3">
                {msg.is_read === false && ( 
                  <button
                    onClick={() => updateStatus.mutate({ id: msg.id, is_read: true })}
                    className="flex items-center gap-2 px-4 py-2 text-sm font-semibold bg-zinc-200 dark:bg-zinc-700 hover:bg-zinc-300 dark:hover:bg-zinc-600 rounded-xl transition-all active:scale-95"
                  >
                ...
                {msg.is_read === true && ( 
                 <button
                    onClick={() => updateStatus.mutate({ id: msg.id, is_read: false })}
                    className="flex items-center gap-2 px-4 py-2 text-sm font-semibold bg-yellow-100 text-yellow-700 hover:bg-yellow-200 dark:bg-yellow-900/30 dark:text-yellow-400 rounded-xl transition-all active:scale-95"
                  >
